/*
 * E906 "LCD gradient" example for the Allwinner A523/T527 RISC-V
 * co-processor (Avaota-A1).
 *
 * Cross-compiled for rv32imac/ilp32.  Drives the on-board ST7789V LCD
 * (240x135) directly from the E906 via S_SPI0, with no ARM/kernel
 * involvement: a moving white bar over an animated RGB gradient, plus a
 * frame counter (bottom-right).
 *
 * The E906 also keeps a small mailbox in the reserved DDR carveout
 * (0x60000000, DA == PA) that the ARM host can watch with:
 *   sudo devmem 0x60000000      # magic   = 0xE9061B0B
 *   sudo devmem 0x6000000C      # frame counter
 *   sudo devmem 0x60000010      # result (0 = ok)
 *
